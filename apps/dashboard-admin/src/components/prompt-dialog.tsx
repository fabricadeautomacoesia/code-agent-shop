...
interface PromptState {
  ...
  mode: 'prompt' | 'confirm';
  title: string;
  body?: string; // FIX-WORKER-4 pass 150: extra description (mensagem secundaria)
  placeholder?: string;
  defaultValue?: string;
  confirmLabel?: string;
...
/**
 * FIX-WORKER-4 pass 150: confirmDialog substitui window.confirm() nativo.
 * Mesma API: title + opt body description -> Promise<boolean>
 *
 * @param title - titulo curto (ex: 'Confirmar acao?')
 * @param opts - body (descricao), confirmLabel, variant=danger p/ acoes destrutivas
 */
...
export function PromptDialogProvider() {
  ...
  useEffect(() => {
    if (state.open) {
      ...
      // FIX-WORKER-4 pass 150: focus apropriado por mode
      setTimeout(() => {
    ...
    >
      <div className="glass-strong rounded-xl max-w-md w-full shadow-2xl">
        ...
        <form onSubmit={onSubmit} className="p-4 space-y-4">
          {/* FIX-WORKER-4 pass 150: body description (multi-line via whitespace-pre-line) */}
          {state.body && (
