...
// FIX-WORKER-18 pass 176: cache 30s vary by user+limit. Chamado em cart/page.tsx
// useEffect a cada render (saldo balance Loyalty Redeem card).
// Pre-fix: 2-3 queries por request (user_loyalty + loyalty_transactions + welcome bonus)
// Post-fix: cache hit ~1ms (Redis) vs ~12ms PG.
// Invalidacao: POST /earn (mesmo svc) + cart redeem cross-svc (deferred via TTL).
//
// SIDE-EFFECT (welcome bonus): apenas roda na 1a chamada do user starter.
// Apos rodar, response sera identica (com pontos) - cache valido.
// Edge case: user starter cria conta -> chama /me (insert bonus + return)
// -> cache armazena resposta com bonus -> chamadas seguintes hit cache ok.
const loyaltyMeCacheKey = (req) => {
  const userId = req.user?.sub || 'anon';
  const lim = req.query.limit || '20';
  return `loyalty:me:${userId}:l${lim}`;
};
